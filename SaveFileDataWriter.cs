...
{
    public string saveDataDirectoryPath = ""; //เก็บ ตำแหน่งโฟลเดอร์
    public string saveFileName = ""; //เก็บ ชื่อไฟล์
    ...
    public void CreateNewCharacterSaveFile(PlayerSaveData playerSaveData)
    {
        ...
        try
        {
            Directory.CreateDirectory(Path.GetDirectoryName(savePath)); //ตัวสร้างโฟลเดอร์ ถ้ามีโฟลเดอร์อยู่แล้วจะไม่สร้างขึ้นมาใหม่
    ...
    public PlayerSaveData LoadSaveFile()
    {
        ...
        if (File.Exists(loadPath))
        {
            ...
            {
                Debug.LogError("ERROR WHILE TRYING TO LOAD PLAYER DATA, GAME NOT SAVED" + loadPath + "\n" + ex);
            }
        }
